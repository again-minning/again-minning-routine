...
@commit
def put_detail_retrospect(db: Session, retrospect_id: int, content: str, image: UploadFile, account: int):
    retrospect = retrospect_join_image_find_by(db, retrospect_id, account)
    if not retrospect:
        raise MinningException(RETROSPECT_NOT_FOUND_ID)
...
@commit
def delete_detail_retrospect(db: Session, retrospect_id: int, account: int):
...
@commit
def get_list_retrospect(db: Session, date: str, account_id: int):
    date = convert_str2datetime(date)
